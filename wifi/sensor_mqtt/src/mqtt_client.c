...
#define USERNAME      "uj742"
#define AIO_KEY       "ADAFRUIT_IO_KEY_REDACTED"
#define FEED_TOPIC    USERNAME "/feeds/audiosatatus"
...
int app_mqtt_init(struct mqtt_client *client)
{
	...
	LOG_INF("Connecting to MQTT broker @ %s", broker_ip);
        ...
        username.utf8 = (uint8_t *)USERNAME;
        username.size = strlen(USERNAME);
        password.utf8 = (uint8_t *)AIO_KEY;
        password.size = strlen(AIO_KEY);
